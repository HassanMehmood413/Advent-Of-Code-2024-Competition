...
function evaluateEquation(numbers, operators) {
    let expression = String(numbers[0]); // Start with the first number as a string
    for (let i = 0; i < operators.length; i++) {
        if (operators[i] === '+') {
            expression = String(Number(expression) + numbers[i + 1]);
        } else if (operators[i] === '*') {
            expression = String(Number(expression) * numbers[i + 1]);
        } else if (operators[i] === '||') {
            expression += String(numbers[i + 1]); // Concatenate the numbers as strings
        }
    }
    return Number(expression); // Convert the final result back to a number
}
